...
namespace QuadCore
{
	...
	class ExtraObject : public iShape
	{
	public:
		ExtraObject(glm::vec3 position, int figure_num)
		{
			this->SetName();
		...
		ExtraObject(std::string mesh_name, std::string shader_name, std::string texture_name, Transform* transform = new Transform(), bool normal_view = false)
		{
	...
	public:
		...
		//	string file_name_shader = "../resources/shaders/" + shader_name;
		//	m_shader = new Shader(file_name_shader, 3);	/// include Geometry Shader
