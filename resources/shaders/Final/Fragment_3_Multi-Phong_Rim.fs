...
void main()
{
	...
	for(int i = 0; i < LIGHTSNUM; i++)
		result += CalcPointLight2(pointLights[i], fs_in.normal0, fs_in.FragPos);
	gl_FragColor = result;
}
...
vec4 CalcPointLight2(PointLight light, vec3 normal, vec3 fragPos)
{
	...
	vec4 ambient = vec4(light.light_ambient * vec3(texture(texture_diffuse, fs_in.texCoord0)), 1.0);

    // Compute the diffuse contribution
    vec4 diffuse = max(0.0, dot(N, L)) * vec4(light.light_diffuse,1.0) * texture2D(texture_diffuse, fs_in.texCoord0);
...
vec3 CalcDirLight(vec3 normal, vec3 viewDir)
{
    ...
    vec3 specular = light_specular * spec  * vec3(texture(texture_diffuse, fs_in.texCoord0));// * vec3(texture(material.specular, texCoord0));
